...
//            System.out.println(Arrays.toString(scores));
    ...
    public int[] getBottomSentence(int size) //获得size数量的打分最低的句子
    {
        Collection<Integer> values = top.values();
        size = Math.min(size, values.size());
        int[] indexArray = new int[size];
        Iterator<Integer> it = values.iterator();
        int[] tempList = new int[values.size()];
        for (int i = 0; i < values.size(); i++) {
            tempList[i] = it.next();
        }
        int[] list = reverseNumArray(tempList);
    ...
    public static int[] reverseNumArray(int[] array){
        int head = 0;
...
//        return getBottomSentenceList(document, size, default_sentence_separator);
    }
    ...
    public static List<String> getBottomSentenceList(String document, int size, String sentence_separator)
    {
        List<String> sentenceList = splitSentence(document, sentence_separator);
        List<List<String>> docs = convertSentenceListToDocument(sentenceList);
        TextRankSentence textRank = new TextRankSentence(docs);
        int[] topSentence = textRank.getBottomSentence(size);
        List<String> resultList = new LinkedList<String>();
        for (int i : topSentence)
        {
            resultList.add(sentenceList.get(i));
